...
- Creation of the WSDM allows for setting the host and port that the device websocket server will
  listen on.
- The WSDM is added to the ButtplugServer during the server configuration step.
- When the server is started, the WSDM will listen on the specified host/port combination.
- At this time, a device can be connected to the WSDM port.
...
This JSON block should be the first thing sent to the websocket server. After this, one of two
things will happen:

- If the server accepts the device, all further communication will be in the context of the
  requested protocol, from its initialization phase.
- If the server rejects the device, the websocket is closed with an error reason.
