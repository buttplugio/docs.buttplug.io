...
Lovense has been manufacturing toys since 2011. While all Lovense toys use the same protocol, they can communicate over Bluetooth differently depending on when they were released. The first toys released by Lovense used both Bluetooth 2.0 SPP (emulating a serial port) and Bluetooth LE. Starting with the Lush, all toys released by Lovense use only Bluetooth LE with GATT characteristics to mimic the RX/TX setup of the serial port style control of the old toys.
  ...
  - uuid: "0000fff0-0000-1000-8000-00805f9b34fb"
    characteristics:
      - uuid: "0000fff1-0000-1000-8000-00805f9b34fb"
        ...
        role: rx
        description: "RX characteristic (first generation)"
      - uuid: "0000fff2-0000-1000-8000-00805f9b34fb"
        properties: [write-without-response, write]
        role: tx
        description: "TX characteristic (first generation)"
  - uuid: "6e400001-b5a3-f393-e0a9-e50e24dcca9e"
    characteristics:
      - uuid: "6e400003-b5a3-f393-e0a9-e50e24dcca9e"
        properties: [notify]
        role: rx
        description: "RX characteristic (second generation)"
      - uuid: "6e400002-b5a3-f393-e0a9-e50e24dcca9e"
        properties: [write-without-response, write]
        role: tx
        description: "TX characteristic (second generation)"
  - uuid: "XY300001-002Z-4bd4-bbd5-a6920e4c5653"
    characteristics:
      - uuid: "XY300003-002Z-4bd4-bbd5-a6920e4c5653"
        properties: [notify]
        role: rx
        description: "RX characteristic (third generation, variable)"
      - uuid: "XY300002-002Z-4bd4-bbd5-a6920e4c5653"
        properties: [write-without-response, write]
        role: tx
        description: "TX characteristic (third generation, variable)"
```

**Notes on service discovery:**

Lovense toy names always start with "LVS-". What comes after that varies depending on when the toy was released. Early toys used names involving the single character identifier, like "LVS-A011", while newer toys use the full product name, like "LVS-Edge36". The last 2 numbers denote the firmware version the toy is running.

Lovense toys usually have one of 3 service ID formats as listed above. The first two service IDs are static, and represent the service IDs used by first and second generation Lovense toys. The 3rd service ID can vary, with:

- X usually being 0x4 or 0x5
- Y being any number 0x0-0xf
- Z usually being 0x3 or 0x4

While some Bluetooth APIs can wildcard services, others like WebBluetooth require an exact service UUID to connect. For these instances, it's recommended to just generate out all 32 variations of the last service, for a total of 34 services, to use with the optionalServices portion of a WebBluetooth connection filter.

To identify the type of toy after connecting, it is recommended to use the `DeviceType;` message, outlined below. This will return a device model identifier.
...
Commands for Lovense toys follow these rules:
...
Returns toy model type, firmware version, and Bluetooth MAC address, as a colon delimited list
...
- Lovense toy names always start with "LVS-" (or older "LOVE-" for Bluetooth 2.0 devices).
- The GATT service and characteristic IDs differ between different toy firmware versions.
- The third service ID format (XY300001-002Z-4bd4-bbd5-a6920e4c5653) can vary in the X, Y, and Z positions.
- It's difficult to keep a current list of exact Lovense device names and service/characteristic UUIDs, as they tend to change rapidly on firmware updates.
- Some Bluetooth APIs can wildcard services, but others like WebBluetooth require an exact service UUID to connect.
