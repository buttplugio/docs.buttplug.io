...
This is not to say that information is not accessible by applications at all. It is only scoped to what the core API, minus raw device commands, allows. If a user allows an application to send [raw commands](./devices-and-commands/raw-commands), the protocol the toy uses may relay sensitive information. For instance, the [Lovense Toy Protocol](/docs/stpihkal/protocols/lovense) will dump the Bluetooth Address as part of the reply to the `DeviceType;` message.
